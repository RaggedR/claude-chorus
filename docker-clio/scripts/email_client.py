#!/usr/bin/env python3
"""
Clio's email client — send and receive emails via Gmail IMAP/SMTP.
...
def extract_attachment_metadata(msg) -> list:
    """Walk a parsed email and return metadata for each attachment."""
    attachments = []
...
def send_email(to_addr: str, subject: str, body: str, reply_to_id: str = None, cc: str = None, attachments: list = None) -> dict:
    ...
    try:
        ...
        with open(log_path, "w") as f:
            json.dump({
                "to": to_addr, "cc": cc, "subject": subject,
                "body": body, "attachments": attached_files,
...
def check_inbox(unread_only: bool = True, limit: int = 10, folder: str = "INBOX") -> list:
    """Check a mailbox folder for messages."""
    try:
...
def read_email(uid: int, folder: str = "INBOX") -> dict:
    ...
    try:
        with IMAPClient(IMAP_HOST, ssl=True) as client:
            ...
            with open(log_path, "w") as f:
                json.dump({
                    "uid": uid, "from": from_addr,
                    "subject": env.subject.decode() if env.subject else "(no subject)",
                    "body": body, "date": str(env.date),
                    "message_id": env.message_id.decode() if env.message_id else None,
                    "attachment_count": len(att_meta), "attachments": att_meta,
                }, f, indent=2)
        ...
        with IMAPClient(IMAP_HOST, ssl=True) as client:
            ...
            for part in msg.walk():
                ...
                with open(file_path, "wb") as f:
                    f.write(payload)
                saved_files.append({
                    "filename": safe_name, "path": file_path,
                    "content_type": content_type,
                    "size": len(payload), "size_human": _format_bytes(len(payload)),
                })
            ...
            return {
                ...
                "saved_to": save_dir, "files": saved_files,
            }
    except Exception as e:
        return {"error": str(e)}
...
def main():
    parser = argparse.ArgumentParser(description="Clio's email client")
