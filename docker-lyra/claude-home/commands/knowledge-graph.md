...
1. Determine the target directory (required):
   ...
   - If `$ARGUMENTS` names a lit-review (e.g., "cyclic-descent-2026-02-16"), resolve to `~/data/lit-reviews/$ARGUMENTS/`
   - If `$ARGUMENTS` is empty, tell the user: "Please provide a directory. Example: `/knowledge-graph /tmp/my-review`"
   - If ambiguous, run `python3 ~/data/lit-reviews/lit_review.py list` to show available reviews

2. Run:

```bash
python3 ~/git/research/bin/build_knowledge_graph.py --dir <target-directory>
```
...
- The script lives at `~/git/research/bin/build_knowledge_graph.py`
- Existing `~/data/arxiv-rag/` scripts are unaffected and work independently
