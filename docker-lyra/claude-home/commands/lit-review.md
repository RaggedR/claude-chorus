...
### Step 0 — Parse arguments and start todo tracker
...
**Create `<dir>/todo.md`** — a running tracker you update after every step. This keeps your goals in recent context during long searches (~50+ tool calls). Format:
```
# Lit Review: <topic>
## Current step: 1 — Mining conversation context
## Papers found: 0
## Queries run: 0
## Key leads: (none yet)
## Next: Generate diversified search queries
```
Update this file at the start and end of every step. It takes 5 seconds and prevents drift.
...
```bash
python3 ~/data/lit-reviews/lit_review.py search "<query>" --dir <dir> --max-papers <N> [--abstracts-only]
```
...
### Important: One sub-agent per paper

**Whenever you need to read a paper** — to check relevance, extract keywords, read the introduction, scan the bibliography, or summarize — **always spawn a sub-agent** to do it. Never read papers directly in your main context. This keeps your orchestrator context clean and prevents quality degradation as the corpus grows.

Sub-agent prompt template:
"First, read ~/.claude/AGENT.md for instructions. Then read this paper and return structured results. Paper: [path or URL]. Task: [what to extract]. Return as JSON with fields: [specify fields]."

For batch operations (e.g., scanning 10 abstracts for new leads), dispatch sub-agents **in parallel** — one per paper, or batch 3-5 abstracts per agent if they're short. The tokens are worth it — a fresh context per paper produces dramatically better extractions than one overloaded context reading 20 papers sequentially.

### Step 4 — Snowball: Scan abstracts for new leads

Read the accumulated results using the Read tool on `<dir>/selected_papers.json`. For papers whose abstracts need closer analysis, **dispatch sub-agents** (one per paper or small batches of 3-5 abstracts per agent). Have each agent analyze the abstracts looking for:
...
Pick the **5-8 highest-relevance PDFs** from `<dir>/papers/`. **Dispatch one sub-agent per paper in parallel.** Each sub-agent reads the first 3-5 pages (introduction + related work) and extracts:

Sub-agent prompt: "First, read ~/.claude/AGENT.md for instructions. Read this PDF: [path]. Read pages 1-5. These sections are expert-curated mini literature reviews. Extract and return as structured text: (1) Key papers described as foundational/motivating, with arXiv IDs if visible, (2) Techniques described as 'the standard approach' or 'state of the art', (3) Open problems or conjectures mentioned, (4) Connections to other fields the authors consider important, (5) Any author names that appear repeatedly in citations."

Collect all sub-agent results and merge into a unified list of leads.

#### 5b — Read reference lists

For the same PDFs, **dispatch one sub-agent per paper in parallel** to read the bibliography (last few pages). Each sub-agent extracts:

Sub-agent prompt: "First, read ~/.claude/AGENT.md for instructions. Read this PDF: [path]. Read the last 5 pages (bibliography/references). Extract and return: (1) Papers cited by title that directly address [user's topic], (2) Recent papers (last 2-3 years) by the same authors as this paper, (3) Any arXiv IDs visible in the references. Return as a structured list."

Merge results across all sub-agents. Papers cited by multiple of your top papers (convergence = importance) are high-priority downloads. For papers with known arXiv IDs, download directly (see Reference below). For others, search arXiv by author + key title words.
...
```bash
python3 ~/data/lit-reviews/lit_review.py cleanup --dir <dir> --keep 50 "overall topic description that captures all facets"
```
...
- Use `python3 ~/data/lit-reviews/lit_review.py list` to see past reviews
